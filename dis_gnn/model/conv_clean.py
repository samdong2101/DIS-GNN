...
class Convolution(MessagePassing):
    def __init__(self, in_dim, 
        ...
        self.node_linears = GatedMLP(node_linear_in_dim, out_dim,hidden_dim = self.resid_hidden, n_layers = self.n_resid_layers) # used to be ResidualMessageMLP
    ...
    def forward(self, node_inp, 
        ...
        return concat_node, concat_edge

    def average_edge_vector(self, 
                            edge_index, 
                            edge_attr):
        src = edge_index[0]  
        dest = edge_index[1] 
        num_nodes = max([int(src.max()) + 1, int(dest.max()) + 1])       
        out = scatter_sum(edge_attr, src, dim=0, dim_size=num_nodes)
    ...
    def message(self, 
        ...
        if graphtype == 'crystal':
            ...
            if cells is not None:
                ...
                parts.append(cells) 
            try:
                concat_node = torch.concat(parts,dim = 1)
            except Exception as e:
                print('ERROR MAKING CONCAT_NODE:', e)
                # print('concat_edge shape:',concat_edge.shape)
...
class GeneralConv(nn.Module):
    def __init__(self, in_hid, 
        ...
        self.base_conv = Convolution(in_hid, 
                                     out_hid, 
                                     n_resid_layers, 
                                     n_mlp_layers, 
                                     graphtype, 
