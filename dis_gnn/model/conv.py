...
class Convolution(MessagePassing):
    def __init__(self, in_dim, out_dim, n_resid_layers = 1, n_mlp_layers = 1, **kwargs):
        ...
        self.node_linears = ResidualMessageMLP(in_dim*2, out_dim,hidden_dim = self.resid_hidden, n_layers = self.n_resid_layers)
        #self.drop = nn.Dropout(dropout)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print('n_mlp_layers:', n_mlp_layers)
    def forward(self, node_inp, edge_index, edge_feature):
        concat_edge, concat_node = self.message(edge_index[0],edge_index[1], node_inp, node_inp, edge_feature)
        return concat_node
    ...
    def message(self, edge_index_i, edge_index_j, node_inp_i, node_inp_j, edge_feature):
        '''
...
class GeneralConv(nn.Module):
    ...
    def forward(self, meta_xs, edge_index, edge_feature):
        return self.base_conv(meta_xs, edge_index, edge_feature)
