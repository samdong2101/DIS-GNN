...
class GNN(nn.Module):
    def __init__(self, 
        ...
        self.edge_linear = nn.Linear(40, n_hid) # --> changed to 40 from 80
        ...
        self.node_ff = mlp(in_dim, 
                           n_hid, 
                           hidden_dim=ff_hidden, 
                           num_layers = n_mlp_layers)
        self.line_node_ff = mlp(line_in_dim, 
                                n_hid, 
                                hidden_dim=ff_hidden,
                                num_layers=n_mlp_layers)
        self.edge_ff = mlp(40, 
                           n_hid, 
                           hidden_dim=ff_hidden, 
                           num_layers = n_mlp_layers) # --> changed to 40 from 80 
        self.line_edge_ff = mlp(40, 
                                n_hid, 
                                hidden_dim=ff_hidden,
                                num_layers = n_mlp_layers) 
        self.state_ff = mlp(batch_size, 
                            batch_size, 
                            hidden_dim = ff_hidden, 
                            num_layers = n_mlp_layers) 
        self.cell_ff = mlp(batch_size*9, 
                           batch_size, 
                           hidden_dim = ff_hidden, 
                           num_layers = n_mlp_layers)
        self.coord_ff = mlp(3, 
        ...
        for l in range(n_layers - 1):   
            ...
            self.line_gcs.append(GeneralConv(n_hid, 
                                             ...
                                             include_cells = self.include_cells, 
                                             include_states = self.include_states))
            self.final_gcs.append(GeneralConv(n_hid, 
                                              n_hid, 
                                              n_resid_layers = n_resid_layers, 
                                              n_mlp_layers = n_mlp_layers, 
                                              include_coords = self.include_coords, 
    ...
    def forward(self, 
                ...
                edge_index,  
                edge_feature, 
                line_node_feature = None, 
                line_edge_index = None, 
                line_edge_feature = None, 
                global_state = None,
                group_size = None, 
                cell = None, 
                coords = None): 
        ...
        if cell is not None:
            ...
            meta_cell = torch.repeat_interleave(meta_cell, group_size).to(self.device)
        else:
            meta_cell = None
        ...
        for i in range(len(self.gcs)):
            ...
            if i == 0:
                meta_node, meta_edge = gc(meta_node, 
                                          edge_index, 
                                          meta_edge, 
                                          meta_state, 
                                          meta_cell, 
                                          meta_coords, 
                                          'crystal')
                line_meta_node, line_meta_edge = lgc(meta_edge, 
                                                     line_edge_index, 
                                                     line_meta_edge, 
                                                     None, 
                                                     None, 
                                                     None, 
                                                     'line')
                final_node, final_edge = fgc(meta_node, 
                                             edge_index, 
                                             line_meta_node, 
                                             meta_state, 
                                             meta_cell, 
                ...
                meta_node, meta_edge = gc(meta_node, 
                                          ...
                                          meta_coords, 
                                          'crystal')
                line_meta_node, line_meta_edge = lgc(line_meta_node, 
                                                     line_edge_index, 
                                                     line_meta_edge, 
                                                     None, 
                                                     None, 
