...
def load_data(structures_path = None, 
              df_path = None, 
              ldf_path = None, 
              batch_size = 32, 
    ...
    if df_path is None:
        ...
        gf = GraphFeaturizer(structures, 
                             cutoff, 
                             property_name, 
                             composition = composition, 
                             num_atoms = num_atoms, 
                             save_path = df_save_path, 
                             scope = scope)
        df = gf.featurize()
        lgf = LineGraphFeaturizer(df, 
                                  save_path=ldf_save_path, 
                                  crystal_save_path = df_save_path)
        df,ldf = lgf.featurize()
        dl = DataLoader(df, 
        ...
        print(f'loading data at {df_path} and {ldf_path}...') 
...
class DataLoader:
  ...
  def load_data(self, 
    ...
    if self.graphtype == 'crystal':
        node_features = [df.iloc[i]['node_feature'] for i in range(len(df))]
        edge_indices = [df.iloc[i]['edge_index'] for i in range(len(df))]
        if labels_list is None:
            labels_list = [df.iloc[i][self.property_name] for i in range(len(df))]
        edge_features = [df.iloc[i]['edge_feature'] for i in range(len(df))]
        
        if self.scope == 'graph':
            if self.task == 'classification':
                labels_list = [df.iloc[i]['classification_labels'] for i in range(len(df))]
            ...
            labels = labels_list
        structures = [df.iloc[i]['structure'] for i in range(len(df))]
        ...
        node_features = [df.iloc[i]['line_node_feature'] for i in range(len(df))]
        edge_indices = [df.iloc[i]['line_edge_index'] for i in range(len(df))]
        edge_features = [df.iloc[i]['line_edge_feature'] for i in range(len(df))]
  ...
  def batch_site_properties(self, 
  ...
  def batch_labels(self, 
      ...
      if self.scope == 'node':
        for batch in range(num_batches):
            ...
            i = i + batch_size
            batched_tensor = torch.cat([torch.tensor(i) for i in batched_target_value])
            batched_tensor = batched_tensor.reshape(1,batched_tensor.shape[0]) 
  ...
  def batch_data(self, 
        ...
        if self.graphtype == 'crystal':
            batched_node_features, batched_group_sizes = self.batch_node_features(node_features, 
                                                                                  self.batch_size)
            batched_edge_indices = self.batch_edge_indices(edge_indices, 
                                                           self.batch_size, 
                                                           node_features)
            batched_edge_features = self.batch_edge_features(edge_features, 
                                                             self.batch_size)
            batched_node_indices = self.batch_node_indices(node_features,
                                                           self.batch_size)
            batched_labels = self.batch_labels(labels, 
                                               self.batch_size)
            batched_cells = self.batch_cells(cells, 
                                             self.batch_size)
            batched_coords = self.batch_coordinates(coords, 
                                                    self.batch_size)
  ...
  def get_data(self):
    if self.graphtype == 'crystal':
        ...
        batched_node_features, batched_edge_indices, batched_edge_features, batched_labels, batched_node_indices, batched_group_sizes, batched_cells, batched_coords = self.batch_data(self.batch_size, 
                                                                                                                                                                                       node_features, 
                                                                                                                                                                                       edge_indices, 
                                                                                                                                                                                       edge_features, 
                                                                                                                                                                                       labels, 
        ...
        batched_node_features, batched_edge_indices, batched_edge_features = self.batch_data(self.batch_size, 
                                                                                             node_features, 
                                                                                             edge_indices, 
                                                                                             edge_features)
        return batched_node_features, batched_edge_indices, batched_edge_features 
