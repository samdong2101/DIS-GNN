...
def train(cfg_path):
    ...
    gnn = GNN(in_dim, 
              ...
              batch_size, 
              dropout=gnn_dropout,
              n_resid_layers=gnn_n_resid_layers, 
              n_mlp_layers = gnn_n_mlp_layers, 
              include_coords = include_coords, 
              include_cells = include_cells, 
              include_states = include_states).to(device) 
    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(range(num), desc = f'Epoch {epoch+1}/{num_epochs}'):
            ...
            if include_states:
                ...
                state = torch.zeros(batch_size).to(dtype = torch.float32, device = device)
            if include_coords:
                coords = batched_coords[i].to(dtype = torch.float32, device = device)
            else:
                coords = None
            if include_cells:
                cells = batched_cells[i].to(dtype = torch.float32,device = device)
            else:
                cells = None
            ...
            output = gnn(flattened_node_features, 
                         flattened_edge_indices, 
                         flattened_edge_features, 
                         global_state = state, 
                         group_size = group_sizes, 
                         cell = cells, 
                         coords = coords, 
                         line_node_feature = flattened_line_node_features, 
                         line_edge_index = flattened_line_edge_indices, 
                         line_edge_feature = flattened_line_edge_features)
    ...
    torch.save(model.state_dict(), os.path.join(ckpt_path, stamp +f'_{epoch+1}_epochs.pt'))
