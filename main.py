...
import yaml
from dis_gnn.models import GNN 
from dis_gnn.models.downstream import classifier 
...
def load_data(structures = None, df_path = None, property_name = 'band_gap'):
      if featurize: 
            gf = GraphFeaturizer(structures, properties, cutoff, property_name)
            df = gf.featurize()
            dl = DataLoader(df)
            batched_node_features, batched_edge_indices, batched_edge_features, batched_labels = dl.get_data()
